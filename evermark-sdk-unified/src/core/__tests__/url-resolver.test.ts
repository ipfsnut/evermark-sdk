 ...
 * These tests ensure the core image handling logic is bulletproof
 */
...
describe('Source Filtering', () => {
  const sampleSources = resolveImageSources({
    supabaseUrl: 'https://supabase.storage.com/image.jpg',
    thumbnailUrl: 'https://supabase.storage.com/thumb.jpg',
    ipfsHash: 'QmYwAPJzv5CZsnA625s3Xf2nemtYgPpHdWEz79ojWnPbdG'
  });

  it('filters for slow network', () => {
    const filtered = filterSourcesByCondition(sampleSources, 'slow-network');
    expect(filtered.every(s => s.type === 'thumbnail' || s.metadata?.size !== 'original')).toBe(true);
  });

  it('avoids IPFS when requested', () => {
    const filtered = filterSourcesByCondition(sampleSources, 'avoid-ipfs');
    expect(filtered.some(s => s.metadata?.storageProvider === 'ipfs')).toBe(false);
  });

  it('prefers thumbnails when requested', () => {
    const filtered = filterSourcesByCondition(sampleSources, 'prefer-thumbnails');
    expect(filtered.every(s => s.type === 'thumbnail' || s.type === 'primary')).toBe(true);
  });
});

describe('Source Validation', () => {
  it('validates correct sources', () => {
    const sources = resolveImageSources({
      supabaseUrl: 'https://supabase.storage.com/image.jpg'
    });
    
    const validation = validateImageSources(sources);
    expect(validation.valid).toBe(true);
    expect(validation.errors).toHaveLength(0);
  });

  it('detects empty source list', () => {
    const validation = validateImageSources([]);
    expect(validation.valid).toBe(false);
    expect(validation.errors).toContain('No image sources provided');
  });
  ...
  it('detects invalid URLs', () => {
    ...
    const validation = validateImageSources(invalidSources);
    expect(validation.valid).toBe(false);
  ...
  it('detects duplicate priorities', () => {
    const duplicateSources = [
      { url: 'https://example.com/1.jpg', type: 'primary' as const, priority: 1 },
      { url: 'https://example.com/2.jpg', type: 'thumbnail' as const, priority: 1 }
    ];
    
    const validation = validateImageSources(duplicateSources);
    expect(validation.valid).toBe(false);
    expect(validation.errors.some(e => e.includes('Duplicate priorities'))).toBe(true);
  });
});
