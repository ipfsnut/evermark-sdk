...
export class EnhancedImageLoader extends ImageLoader {
  private orchestrator: StorageOrchestrator | null = null;
  private autoTransfer: boolean;
  private onStorageProgress?: (progress: UploadProgress) => void;
  ...
  async loadImageWithStorageFlow(input: ImageSourceInput): Promise<LoadImageResult> {
    try {
      ...
      if (this.orchestrator && this.autoTransfer) {
        ...
        const flowResult = await this.orchestrator.ensureImageInSupabase(
          input, 
          this.onStorageProgress
        );
        
        // Update input with the result from storage flow
        const enhancedInput: ImageSourceInput = {
          ...input,
   ...
   */
  async runStorageFlow(input: ImageSourceInput): Promise<StorageFlowResult> {
   ...
   */
  async testStorageStatus(): Promise<{
    available: boolean;
